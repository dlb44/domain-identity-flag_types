...
function testResponseError(res, returnCode) {
  ...
  if (res.body) {
    ...
    expect(body.validation_response.return_code).to.equal(returnCode);
  }
}
...
function testMockResponseBasicSuccess(res) {
  ...
  if (res.body) {
    ...
    let body = JSON.parse(res.body);
    expect(body.basic.metadata.validation_response.return_code).to.equal(200);
  }
}
...
function testMockResponsePOSTSuccess(res) {
  ...
  if (res.body) {
    let body = JSON.parse(res.body);
    expect(body.metadata.validation_response.return_code).to.equal(200);
  }
}
...
describe('Test Helper Functions Unit Tests', function () {
  // ----- testResponseError -----
  it('testResponseError should verify that the statusCode and the validation_response.return_code match the given returnCode', () => {
    testResponseError({ statusCode: 200, body: JSON.stringify({ validation_response: { return_code: 200 } }) }, 200);
  });
  ...
  it('testMockResponseBasicSuccess should verify that the statusCode and the validation_response.return_code match the given returnCode', () => {
    testMockResponseBasicSuccess({ statusCode: 200, body: JSON.stringify({ basic: { metadata: { validation_response: { return_code: 200 } } } }) }, 200);
  });
  ...
  it('testMockResponsePOSTSuccess should verify that the statusCode and the validation_response.return_code match the POST success codes', () => {
    testMockResponsePOSTSuccess({ statusCode: 201, body: JSON.stringify({ metadata: { validation_response: { return_code: 200 } } }) });
  })
});
